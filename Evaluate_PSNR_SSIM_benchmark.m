...
methods = {'wsrx2'};   % 文件夹路径
dataset = {'Set5', 'Set14', 'B100', 'Urban100'};               % 测试数据路径
ext = {'.png'};
num_method = length(methods);
num_set = length(dataset);
record_results_txt = ['PSNR_SSIM_Results_x2_wsr', degradation,'_model.txt'];
...
for idx_method = 1:num_method        % 不同的模型
    
    for idx_set = 1:num_set          % 不同的测试数据集
        fprintf(results, '\n******************************************************************\n');
        ...
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                hr_filepaths = cat(1, filepaths, dir(fullfile( 'experiment', methods{idx_method}, dataset{idx_set}, 'x2', ['*HR',ext{idx_ext}])));
                lr_filepaths = cat(1, filepaths, dir(fullfile( 'experiment', methods{idx_method}, dataset{idx_set}, 'x2', ['*LR',ext{idx_ext}])));
                sr_filepaths = cat(1, filepaths, dir(fullfile( 'experiment', methods{idx_method}, dataset{idx_set}, 'x2', ['*SR',ext{idx_ext}])));
            end
            
            % for idx_im = 1:length(hr_filepaths)
            %     fprintf('|  %s  | |  %s  | |  %s  | \n',hr_filepaths(idx_im).name,lr_filepaths(idx_im).name,sr_filepaths(idx_im).name);
            % end
            ...
            for idx_im = 1:length(hr_filepaths)
                name_HR = hr_filepaths(idx_im).name;
                name_SR = strrep(name_HR, 'HR', 'SR');
                im_HR = imread(fullfile('experiment', methods{idx_method}, dataset{idx_set}, 'x2', name_HR));
                im_SR = imread(fullfile('experiment', methods{idx_method}, dataset{idx_set}, 'x2', name_SR));
