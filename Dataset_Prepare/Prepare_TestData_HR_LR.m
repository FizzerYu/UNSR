...
for idx_set = 1:length(dataset)
    ...
    for idx_im = 1:length(filepaths)
        ...
        if size(im_ori, 3) == 1   % only 1 deminson
        ...
        for scale = scale_all
            ...
            elseif strcmp(degradation, 'DN')
                randn('seed',0); % For test data, fix seed. But, DONT fix seed, when preparing training data.
            ...
            fn_HR = fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], [name_im]);
            fn_LR = fullfile(['./LR/LR', degradation], dataset{idx_set}, ['x', num2str(scale)], [name_im]);
